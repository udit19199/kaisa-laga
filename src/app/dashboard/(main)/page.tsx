...
const SentimentLineChart = dynamic(
  () => import("@/components/dashboard/overview-charts").then((m) => m.SentimentLineChart),
  { ssr: false, loading: () => <Skeleton className="h-64 w-full" /> }
);

const CategoryBarChart = dynamic(
  () => import("@/components/dashboard/overview-charts").then((m) => m.CategoryBarChart),
  { ssr: false, loading: () => <Skeleton className="h-64 w-full" /> }
);
...
function KPICardGrid({
  ...
  positiveRate: string;
  activeLocationsStr: string;
  totalNegative: number;
  totalFeedbackSeries: number[];
  positiveRateSeries: number[];
  dailyActiveLocationsSeries: number[];
...
function LocationsBreakdownCard({
  loading,
  locationsBreakdown,
  locations,
  router,
  ...
  loading: boolean;
  locationsBreakdown: LocationBreakdownItem[];
  locations: Location[];
...
export default function OverviewPage() {
  ...
  const [state, setState] = useState({
    ...
    days: "30",
    sentimentData: [] as Array<{ date: string; Positive: number; Neutral: number; Negative: number }>,
    locationsBreakdown: [] as LocationBreakdownItem[],
    categoryData: [] as Array<{ tag: string; count: number }>,
    loading: true,
  });
  ...
  const fetchData = useCallback(async () => {
    ...
    let nextSentimentData: typeof sentimentData | null = null;
    let nextLocationsBreakdown: LocationBreakdownItem[] | null = null;
    let nextCategoryData: typeof categoryData | null = null;

    if (locRes.ok) {
      const data = await locRes.json();
      nextLocations = data.locations ?? [];
    }
    if (sentRes.ok) {
      const data = await sentRes.json();
      nextSentimentData = data.series ?? [];
      nextLocationsBreakdown = data.locationsBreakdown ?? [];
    }
    if (catRes.ok) {
      const data = await catRes.json();
      nextCategoryData = data.categories ?? [];
    }

    setState((prev) => ({
      ...prev,
      locations: nextLocations ?? prev.locations,
      sentimentData: nextSentimentData ?? prev.sentimentData,
      locationsBreakdown: nextLocationsBreakdown ?? prev.locationsBreakdown,
      categoryData: nextCategoryData ?? prev.categoryData,
      loading: false,
    }));
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <div className={`${overviewSectionClass} p-5 md:p-6`}>
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Select value={days} onValueChange={(v) => v && setState((prev) => ({ ...prev, days: v }))}>
            ...
            <Select
              ...
              onValueChange={(v) => setState((prev) => ({ ...prev, selectedLocation: v ?? "all" }))}
            >
      ...
      <KPICardGrid
        ...
        activeLocationsStr={activeLocationsStr}
        totalNegative={totalNegative}
        totalFeedbackSeries={totalFeedbackSeries}
        positiveRateSeries={positiveRateSeries}
        dailyActiveLocationsSeries={dailyActiveLocationsSeries}
        negativeSeries={negativeSeries}
        days={days}
        loading={loading}
      />
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <TrendCard loading={loading} sentimentData={sentimentData} />
      ...
      <InsightsCard loading={loading} categoryData={categoryData} days={days} />
    </div>
  );
}
...
function TrendCard({ loading, sentimentData }: TrendCardProps) {
  return (
...
interface InsightsCardProps {
  loading: boolean;
  categoryData: Array<{ tag: string; count: number }>;
  days: string;
}

function InsightsCard({ loading, categoryData, days }: InsightsCardProps) {
  return (
