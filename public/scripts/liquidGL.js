 ...
 * ------------------------------------------------*/
 ...
 try {
 let snapTarget = this.snapshotTarget;
 let fullW = snapTarget.scrollWidth;
 let fullH = snapTarget.scrollHeight;
 if (!fullW || !fullH) {
 snapTarget = document.body;
 fullW = snapTarget.scrollWidth || window.innerWidth;
 fullH = snapTarget.scrollHeight || window.innerHeight;
 }
 ...
 const snapshotResolution = this._snapshotResolution ?? 1;
 let scale = Math.min(
 snapshotResolution,
 fullW > 0 ? maxTex / fullW : snapshotResolution,
 fullH > 0 ? maxTex / fullH : snapshotResolution
 );
 if (!Number.isFinite(scale) || scale <= 0) {
 scale = snapshotResolution;
 }
 ...
 const snapCanvas = await html2canvas(snapTarget, {
 allowTaint: false,
 useCORS: true,
 backgroundColor: null,
 removeContainer: true,
 width: Math.max(1, Math.round(fullW)),
 height: Math.max(1, Math.round(fullH)),
 scrollX: 0,
 scrollY: 0,
 scale: Number(scale),
 ...
 html2canvas(el, {
 backgroundColor: null,
 scale: Number.isFinite(this.scaleFactor) ? this.scaleFactor : 1,
 useCORS: true,
 removeContainer: true,
 logging: false,
 ignoreElements: (n) =>
